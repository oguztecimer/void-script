...
- **DOC-02: Fixed stdlib list inconsistencies** — All 4 doc files now list the correct 14 stdlib functions (`echo`, `len`, `range`, `abs`, `min`, `max`, `int`, `float`, `str`, `type`, `percent`, `scale`, `random`, `wait`). Added `float` to CLAUDE.md, `random`+`wait` to grimscript-system.md and modding.md. Moved `wait()` from "Output" to "Control" section in grimscript.md.
...
#### Changed
- **S-46: `echo()` is now instant** — `echo()` no longer consumes a tick. It is handled in `try_handle_instant()` alongside queries, emitting a `ScriptOutput` event and returning control to the executor immediately. Players can now use `echo()` freely for debugging without slowing their entities. Previously, `echo()` was routed through `resolve_action()` as a tick-consuming action.
...
- **S-36: Remove all game builtins** — Removed all 23 hardcoded game builtin commands (queries: `scan`, `nearest`, `distance`, `get_pos`, `get_health`, `get_shield`, `get_target`, `has_target`, `get_type`, `get_name`, `get_owner`, `get_resource`, `get_stat`, `get_custom_stat`, `get_types`, `has_type`; actions: `move`, `attack`, `flee`, `wait`, `set_target`; instant effects: `gain_resource`, `try_spend_resource`) from IR instructions, executor, query functions, resolve_action, compiler, interpreter stubs, and mod.toml. Only stdlib, echo, and data-driven custom commands remain. Commands will be reimplemented from scratch as needed. -1786 lines.
...
#### Added
- **S-23: Main brain** — Entity-less built-in brain stored as `SimWorld.main_brain: Option<ScriptState>`. Runs first each tick (before entity shuffle), using sentinel `EntityId(0)`. Can call resource ops, queries, echo, and custom commands. Cannot perform entity actions (move, attack, flee). Terminal commands execute against the main brain.
