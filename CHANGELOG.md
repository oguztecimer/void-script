...
- **S-36: Remove all game builtins** — Removed all 23 hardcoded game builtin commands (queries: `scan`, `nearest`, `distance`, `get_pos`, `get_health`, `get_shield`, `get_target`, `has_target`, `get_type`, `get_name`, `get_owner`, `get_resource`, `get_stat`, `get_custom_stat`, `get_types`, `has_type`; actions: `move`, `attack`, `flee`, `wait`, `set_target`; instant effects: `gain_resource`, `try_spend_resource`) from IR instructions, executor, query functions, resolve_action, compiler, interpreter stubs, and mod.toml. Only stdlib, print, and data-driven custom commands remain. Commands will be reimplemented from scratch as needed. -1786 lines.
