...
- **S-11: Resource availability gating** — Global resources now have an available/unavailable mechanic mirroring the command availability system. Mods define initially available resources via `[resources] initial = [...]` in `mod.toml`. If `initial` is omitted, all pool resources are available by default. At runtime, calling `get_resource()`, `gain_resource()`, or `try_spend_resource()` on an unavailable resource produces a runtime error ("resource 'X' is not available yet"). In dev mode, all resources are available (gate bypassed). Available resource names are sent to the frontend via IPC alongside available commands. The `[resources]` section in `mod.toml` has been restructured: resource definitions move to `[resources.pool]` and initial availability goes in `[resources] initial`.
