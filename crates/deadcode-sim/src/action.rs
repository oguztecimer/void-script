...
pub fn resolve_action(
    ...
    match action {
        ...
        UnitAction::Custom { name, args } => {
            ...
            // Try Lua handler first (if registered).
            let mut lua_handled = false;
            if world.command_handler.is_some() {
                // Take the handler out to avoid borrow conflicts.
                ...
                match result {
                    ...
                    CommandHandlerResult::Yielded { events: cmd_events, handle, remaining_ticks, interruptible } => {
                        ...
                        if let Some(entity) = world.get_entity_mut(entity_id) {
                            entity.active_channel = Some(crate::entity::ActiveChannel::Lua(
                                crate::entity::LuaCoroutineState {
                                    handle,
            ...
            if !lua_handled {
                // Check if this is a phased command (TOML fallback).
                if let Some(phases) = world.custom_command_phases.get(&name).cloned() {
                    if !phases.is_empty() {
                        if let Some(entity) = world.get_entity_mut(entity_id) {
                            entity.active_channel = Some(crate::entity::ActiveChannel::Toml(crate::entity::ChannelState {
                                command_name: name,
                                args,
                                phases,
                                phase_index: 0,
...
// ---------------------------------------------------------------------------
// CommandHandler trait — allows Lua (or other runtimes) to handle commands
// ---------------------------------------------------------------------------
...
pub enum CommandHandlerResult {
    ...
    /// Command name not handled by this handler (fall through to TOML).
    NotHandled,
